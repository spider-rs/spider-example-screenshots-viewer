...
const SearchBar = ({ setDataValues }: { setDataValues: Dispatch<any> }) => {
  ...
  useEffect(() => {
    setCrawlLimit(loadDefaultCrawlLimit());
  }, []);
      ...
      if (res.ok) {
        ...
        if (reader) {
          while (true) {
            ...
            if (done) {
              processChunk(buffer, true); // Final processing of remaining data
              break;
            }
  ...
  const processChunk = (buffer: string, force: boolean = false) => {
    ...
    while (endIndex < buffer.length) {
      const char = buffer[endIndex];

      if (char === '"' && buffer[endIndex - 1] !== "\\") {
        inString = !inString;
      } else if (!inString) {
        if (char === "{") {
    ...
    const remainingBuffer = buffer.slice(startIndex);

    requestAnimationFrame(() => {
      setDataValues((prevData: any) => {
        if (Array.isArray(prevData)) {
          return [...prevData, ...JSONObjects];
        }
        return JSONObjects;
      });
    });

    return force ? "" : remainingBuffer;
  };
          ...
          <div className="flex gap-3 items-center">
            <div>
              <svg
                ...
                className={
                  "fill-[#3bde77] transition-all group-hover:scale-110"
                }
              >
            ...
            <div>
              <h1 className="text-xl">Spider Crawler Screenshots</h1>
              <p className="text-sm">
                App made for{" "}
                <a
